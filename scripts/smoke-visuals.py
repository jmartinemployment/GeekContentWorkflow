#!/usr/bin/env python3
"""Signed-in smoke: generate pillar → generate visual → see <img> on companion."""
...
ENV = ROOT / ".env.local"
OUT = Path("/tmp/gcw-smoke-visuals")
...
def main() -> int:
    ...
    client_name = f"Visual Smoke {stamp}"
    campaign_name = f"Visual Camp {stamp}"
    keyword = f"visual-kw-{stamp}"
    angle = f"Visual angle {stamp}"
    ...
    with sync_playwright() as p:
        ...
        page.fill('textarea[name="audienceProfile"]', "Visual smoke audience")
        page.select_option('select[name="buyingStage"]', "awareness")
        page.fill('input[name="angle"]', angle)
        page.fill('input[name="callToAction"]', "See the visual")
        ...
        print("3) generate visual")
        form = page.locator('form:has(button:has-text("Generate visual"))')
        if form.count() == 0:
            print("FAIL visual form missing")
            print(page.inner_text("body")[:1200])
            shot(page, "visual-missing")
            return 1
        form.locator('select[name="useCase"]').select_option("social-linkedin")
        form.locator('select[name="provider"]').select_option("openai")
        form.locator('button:has-text("Generate visual")').click()

        try:
            page.wait_for_url(
                re.compile(r"/app/assets/[0-9a-fA-F-]{36}\?.*packKind=visual"),
                timeout=300000,
            )
        except PlaywrightTimeout:
            if "error=" in page.url:
                print("FAIL visual error", page.url)
                shot(page, "visual-error")
                return 1
            print("FAIL wait for visual companion", page.url)
            shot(page, "visual-timeout")
            return 1
        ...
        imgs = page.locator('img[alt], img[src^="data:image"]')
        if imgs.count() == 0:
            # also accept any img in draft preview
            imgs = page.locator("img")
        if imgs.count() == 0:
            print("FAIL no image rendered")
            print(page.inner_text("body")[:1200])
            return 1
