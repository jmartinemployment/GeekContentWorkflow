#!/usr/bin/env python3
"""Signed-in smoke: Assets → version → comment → resolve → approval."""
...
def main() -> int:
    ...
    client_name = f"Asset Smoke {stamp}"
    campaign_name = f"Asset Camp {stamp}"
    keyword = f"asset-kw-{stamp}"
    asset_name = f"Pillar {stamp}"
    comment = f"Tighten lede {stamp}"
    ...
    with sync_playwright() as p:
        ...
        print("4) save version")
        page.click('button:has-text("Save version")')
        page.wait_for_load_state("networkidle")
        page.wait_for_timeout(1500)
        if page.locator("text=v1").count() == 0:
            print("FAIL version", page.inner_text("body")[-600:])
            return 1

        print("5) comment + resolve")
        page.fill('textarea[name="content"]', comment)
        page.click('button:has-text("Add comment")')
        page.wait_for_load_state("networkidle")
        page.wait_for_timeout(1500)
        if page.locator(f"text={comment}").count() == 0:
            print("FAIL comment")
            return 1
        page.fill('input[name="resolution"]', "Done")
        page.click('button:has-text("Resolve")')
        page.wait_for_load_state("networkidle")
        page.wait_for_timeout(1500)
        if page.locator("text=/Resolved:/i").count() == 0:
            print("FAIL resolve", page.inner_text("body")[-600:])
            return 1

        print("6) approval event")
        page.select_option('select[name="action"]', "approved")
        page.click('button:has-text("Record approval event")')
        page.wait_for_load_state("networkidle")
        page.wait_for_timeout(1500)
        body = page.inner_text("body")
        if "approved" not in body.lower():
            print("FAIL approval", body[-600:])
            return 1
