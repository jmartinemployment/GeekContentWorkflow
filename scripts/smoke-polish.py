#!/usr/bin/env python3
"""Signed-in smoke: generate draft → polish panel → ship-check gate."""
...
def main() -> int:
    ...
    client_name = f"Polish Smoke {stamp}"
    campaign_name = f"Polish Camp {stamp}"
    keyword = f"polish-kw-{stamp}"
    angle = f"Polish angle {stamp}"
    ...
    with sync_playwright() as p:
        ...
        page.fill('textarea[name="audienceProfile"]', "Polish smoke audience")
        page.select_option('select[name="buyingStage"]', "awareness")
        page.fill('input[name="angle"]', angle)
        page.fill('input[name="callToAction"]', "Try polish")
        ...
        print("3) polish panel")
        body = page.inner_text("body")
        if "Polish · v" not in body and "Polish ·" not in body:
            print("FAIL polish panel missing")
            print(body[:1200])
            shot(page, "polish-missing")
            return 1
        if "Ship ready" not in body and "Ship check blocked" not in body:
            print("FAIL ship status missing")
            shot(page, "polish-no-ship")
            return 1
        print("   polish panel ok")
        shot(page, "02-polish")

        apply = page.locator('form:has(button:has-text("Apply polish fixes"))')
        if apply.count():
            print("4) apply polish fixes")
            apply.locator('button:has-text("Apply polish fixes")').click()
            ...
            while time.time() < deadline:
                ...
                if page.locator("text=v2").count():
                    break
                if "error=" in url:
                    print("FAIL polish apply error", url)
                    shot(page, "polish-apply-error")
                    return 1
                page.wait_for_timeout(2000)
            else:
                print("FAIL polish apply timeout", page.url)
                shot(page, "polish-apply-timeout")
                return 1
            page.wait_for_load_state("networkidle")
            shot(page, "03-applied")
            print("   apply ok")
        else:
            print("   no Apply polish fixes (score 100) — ok")

        if forbidden:
            print("FAIL content-writer/v3 calls:", forbidden[:3])
            return 1

        print("PASS smoke-polish")
        browser.close()
        return 0
