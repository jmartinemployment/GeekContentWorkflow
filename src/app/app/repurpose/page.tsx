...
async function repurposeAction(formData: FormData) {
  "use server";
  ...
  try {
    ...
    redirect(
      `/app/repurpose?clientId=${clientId}&campaignId=${campaignId}&error=${encodeURIComponent(msg)}`,
    );
  }
}
...
export default async function RepurposePage({
  ...
  try {
    ...
    error = e instanceof Error ? e.message : "Failed to load repurpose";
  }
  ...
  return (
    <div className="mx-auto max-w-4xl px-8 py-10">
      ...
      <p className="mt-2 text-gcw-muted">
        Turn a pillar draft into a channel pack: social posts and ad variants,
        each saved as companion assets you can edit, review, and approve.
      </p>
      ...
      <div className="mt-8 flex flex-wrap gap-3 text-sm">
        {clients.map((c) => (
          <Link
            key={c.id}
            href={`/app/repurpose?clientId=${c.id}`}
            className={
      ...
      {selectedClientId ? (
        <div className="mt-4 flex flex-wrap gap-3 text-sm">
          {campaigns.map((c) => (
            <Link
              key={c.id}
              href={`/app/repurpose?clientId=${selectedClientId}&campaignId=${c.id}`}
              className={
                ...
                >
                  ...
                  <fieldset className="space-y-2">
                    <legend className="text-xs font-medium text-gcw-zinc">
                      Channels (leave empty for full default pack ≈12)
                    </legend>
