...
async function createProjectAction(formData: FormData) {
  ...
  const department = String(formData.get("department") || "marketing") as Department;
  ...
  return (
    <div className="mx-auto max-w-4xl px-8 py-10">
      ...
      <p className="mt-2 text-gcw-muted">
        CWV2 <code className="text-xs">/api/projects</code> — keyword + site URL per
        client, then crawl and generate.
      </p>
      ...
      <ul className="mt-8 space-y-3">
        {visible.map((p) => (
          <li
            key={p.id}
            className="flex items-center justify-between gap-3 rounded-xl border border-gcw-line bg-white px-4 py-3"
          >
            <div>
              <p className="font-medium">{p.name}</p>
              <p className="text-xs text-gcw-zinc">
                {p.targetKeyword} · {p.department} · {p.status}
              </p>
            </div>
            <Link
